function [out_concated] = block_product_adc_(G0pn,G1pn,Y_bin,signY,division,block_b_y,ratio_reram,volt,block_size,adc_bit)
    % [m,n,b] x [n,1,b] = [m,1,b]
    
    %  G0pn, G1pn are the reram values with read and write noise
    %  block_size is the divided block size
    
    [m0,nx0,bx0] = size(G0pn);
    [m1,nx1,bx1] = size(G1pn);
    [ny,k,by] = size(Y_bin);
    ...
    for i = 1:bx0%loop x
        ...
        for j = 1:by%loop y
            ...
            for t = 1:batch_number
                if t == batch_number
                    temp_out = (G0pn(:,(1+(t-1)*block_size):(end),i) * (Y_bin_signed((1+(t-1)*block_size):(end),:,j)))-(G1pn(:,(1+(t-1)*block_size):(end),i) * (Y_bin_signed((1+(t-1)*block_size):(end),:,j)));
                else
                    temp_out = (G0pn(:,(1+(t-1)*block_size):(t*block_size),i) * (Y_bin_signed((1+(t-1)*block_size):(t*block_size),:,j)))-(G1pn(:,(1+(t-1)*block_size):(t*block_size),i) * (Y_bin_signed((1+(t-1)*block_size):(t*block_size),:,j)));
                end
